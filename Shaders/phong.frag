...
#ifdef VULKAN
layout(set=0,location = 0) in vec3 o_normal;
layout(set=0,location = 1) in vec3 o_toLight; // direction to light
...
//TODO: remove these, use uniforms instead (see above)
    vec3 u_lightAmbientIntensity = vec3(0.5, 0.5, 0.5);
    vec3 u_lightDiffuseIntensity = vec3(0.5, 0.5, 0.5);
    ...
    vec3 u_matSpecularReflectance = vec3(0.0, 0.0, 0.0);
...
void main(void)
{
    ...
    vec3 diffuseColor = texture(u_diffuseTexture, o_texcoords).rgb + o_colour.rgb;
