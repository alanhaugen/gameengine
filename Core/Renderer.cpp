...
void Renderer::initVulkan()
{
	...
	VulkanHelper::Init(device, physicalDevice);
	...
	shadowRenderer = new ShadowRenderer(device, lightDescriptorSet, lightDescriptorSetLayout);
...
void Renderer::cleanupLightAndShadow()
{
	...
	vkFreeMemory(device, lightUniformBufferMemory, nullptr);
...
void Renderer::createLightDescriptorSetLayout()
{
	...
	std::array<VkDescriptorSetLayoutBinding, 2> lightBindings = { lightUboLayoutBinding, shadowSamplerBinding };
	...
	if (vkCreateDescriptorSetLayout(device, &lightLayoutInfo, nullptr, &lightDescriptorSetLayout) != VK_SUCCESS) {
...
void Renderer::createLightDescriptorSet()
{
	...
	descriptorSetAllocInfo.descriptorPool = lightDescriptorPool;
	descriptorSetAllocInfo.descriptorSetCount = 1;
	descriptorSetAllocInfo.pSetLayouts = &lightDescriptorSetLayout;;

	if (vkAllocateDescriptorSets(device, &descriptorSetAllocInfo, &lightDescriptorSet) != VK_SUCCESS)
	{
	...
	imageInfo.imageView = shadowRenderer->shadowMapImageView; //todo idk, is that right?
	imageInfo.sampler = shadowRenderer->shadowMapSampler;
...
{
	auto vertShaderCode = VulkanHelper::ReadFile(PATH + "Shaders/Phong.vert.spv");
	auto fragShaderCode = VulkanHelper::ReadFile(PATH + "Shaders/Phong.frag.spv");
	...
	pipelineLayoutInfo.setLayoutCount = static_cast<int32_t>(setLayouts.size());
...
void Renderer::createCommandBuffers()
{
	...
	for (size_t i = 0; i < staticCommandBuffers.size(); i++)
	{
		...
		for (size_t j = 0; j < mStaticRenderComponents.size(); j++)
		{
			...
			vkCmdBindDescriptorSets(staticCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 1, 1, &lightDescriptorSet, 0, nullptr);
...
void Renderer::updateLightUniformBuffer()
{
	...
	auto lightPos = glm::vec3{ (cos(time)), abs(sin(time)), 0 };
	auto view = glm::lookAt(lightPos, { 0.0f, 0.0f, 0.0f }, { 0, 0, 1 }); // todo make directional light
	auto proj = glm::perspective(glm::radians(45.0f), shadowRenderer->shadowMapWidth / (float)shadowRenderer->shadowMapHeight, 0.1f, 10.0f); //todo that is a bit messy, should also be decoupled in the shadow renderer

	ubo.lightViewProj = proj * view;
	void* data;
	vkMapMemory(device, lightUniformBufferMemory, 0, sizeof(ubo), 0, &data);
	memcpy(data, &ubo, sizeof(ubo));
...
void Renderer::drawFrame()
{
	...
	shadowAllocInfo.commandPool = commandPool;
	shadowAllocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
	shadowAllocInfo.commandBufferCount = 1;

	if (vkAllocateCommandBuffers(device, &shadowAllocInfo, &shadowCommandBuffer) != VK_SUCCESS)
		throw std::runtime_error("failed to allocate command buffers!");

	shadowRenderer->RecordCommandBuffer(mDynamicRenderComponents, mDynamicTransformComponents, mMeshes, shadowCommandBuffer);
	...
	vkQueueSubmit(graphicsQueue, 1, &shadowSubmitInfo, inFlightFences[currentFrame]);
	...
	for (size_t j = 0; j < mDynamicRenderComponents.size(); j++)
	{
		...
		vkCmdBindDescriptorSets(dynamicCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 1, 1, &lightDescriptorSet, 0, nullptr);
