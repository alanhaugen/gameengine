...
void Renderer::initVulkan()
{
	...
	createDescriptorPool(); // Descriptor pool must be created before createTextureResource
    
    for (size_t i = 0; i < mEngine->mTextureManager->mAssets.size(); i++)
        createTextureResource(mEngine->mTextureManager->mAssets[i]);

    for (size_t i = 0; i < mEngine->mMeshManager->mAssets.size(); i++) 
        createVertexBuffer(mEngine->mMeshManager->mAssets[i]);

    for (size_t i = 0; i < mEngine->mMeshManager->mAssets.size(); i++)
        createIndexBuffer(mEngine->mMeshManager->mAssets[i]);
...
void Renderer::cleanup()
{
    ...
    for (gea::Texture* texture : mEngine->mTextureManager->mAssets)
    {
    ...
    for (gea::Mesh* mesh : mEngine->mMeshManager->mAssets)
    {
...
void Renderer::createDescriptorSetLayout()
{
    ...
    if (vkCreateDescriptorSetLayout(device, &layoutInfo2, nullptr, &textureSetLayout) != VK_SUCCESS)
        ...
        qDebug("created textureSetLayout");
}

void Renderer::createGraphicsPipeline()
{
    auto vertShaderCode = readFile(PATH + "Shaders/Phong.vert.spv");
    auto fragShaderCode = readFile(PATH + "Shaders/Phong.frag.spv");
    ...
    std::vector<VkDescriptorSetLayout> setLayouts = {descriptorSetLayout, textureSetLayout};
...
void Renderer:: createTextureResource(gea::Texture *texture)
{
    ...
    //MAKE VULKAN IMAGE
    createImage(texture->mTexWidth, texture->mTexHeight, texture->mMipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, texture->mTextureImage, stagingBufferMemory);
    ...
    // Create and store the texture descriptor set (set 1 layout)
...
//     createImage(texWidth, texHeight, texture->mMipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, texture->mTextureImage, stagingBufferMemory);
...
{
    const int MAX_TEXTURES = 5000;
    std::array<VkDescriptorPoolSize, 2> poolSizes{};
    poolSizes[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    poolSizes[0].descriptorCount = static_cast<uint32_t>(swapChainImages.size());
    poolSizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    poolSizes[1].descriptorCount = static_cast<uint32_t>(mEngine->mTextureManager->mAssets.size()) * std::max(1, MAX_FRAMES_IN_FLIGHT);
...
void Renderer::createDescriptorSets(gea::Texture &texture)
{
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++)
    {
        ...
        descriptorWrite.dstBinding = 0; // UBO binding in set 0
...
void Renderer::drawFrame()
{
    ...
    for (size_t j = 0; j < mEngine->mDynamicRenderComponents.size(); j++)
    {
        gea::RenderComponent &renderComponent = mEngine->mDynamicRenderComponents[j];
        gea::Mesh* mesh = mEngine->mMeshManager->mAssets[renderComponent.meshIndex];
        VkBuffer vertexBuffer = mesh->mVertexBuffer;
        VkBuffer indexBuffer = mesh->mIndexBuffer;

        //this will be corrupt if the indices are more than uint32_t!
        uint32_t noOfIndices = mesh->mIndices.size();
        VkDeviceSize offsets[] = { 0 };

        glm::mat4 model = mEngine->mTransformComponents[j].getModelMatrix();
...
VkDescriptorSet Renderer::createTextureDescriptor(gea::Texture &texture)
{
    VkDescriptorSetAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = descriptorPool;
